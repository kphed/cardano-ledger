...
serializeWith :: Bi a => Int -> Int -> a -> LByteString
...
deserialize :: Bi a => LByteString -> D.Decoder s a
deserialize = toCborError . decodeFull

-- | Strict version of `deserialize`.
deserialize' :: Bi a => ByteString -> D.Decoder s a
...
decodeFull :: forall a . Bi a => LByteString -> Either DecoderError a
decodeFull = decodeFullDecoder (label $ Proxy @a) decode

decodeFull' :: forall a . Bi a => ByteString -> Either DecoderError a
decodeFull' = decodeFull . BSL.fromStrict

decodeFullDecoder
  :: Text -> (forall s . D.Decoder s a) -> LByteString -> Either DecoderError a
